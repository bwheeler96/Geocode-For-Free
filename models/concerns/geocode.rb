module Geocode
  ...
  def batch_geocode(results)
    locations = results.pluck(:location).join(',').split(',').select { |x| x.length > 4 && !x.in?(us_states.map{ |y| y[1] && !x.in?(us_states.map { |y| y[0] }) }) }
    ...
    results.find_each do |r|
      next if r.location == 'EXACT' || r.location.length < 8 # Get rid of locations we can't possibly find
      ...
      if city.nil?
        if !state.nil?
          ...
          r.longitude = state[3]
          r.save
          puts "STATE: #{state[0]} for #{r.location}"
          next
        end
    ...
    puts "%#{100 * results.geo.count.to_f / results.count} Coverage"
  end
  ...
  def get_state(words)
    words.each do |w|
      state = us_states.detect { |x| x[0].downcase == w.strip || x[1].downcase == w.strip }
